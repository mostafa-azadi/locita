 ...
 */
public class OwnLocation implements GoogleApiClient.OnConnectionFailedListener, GoogleApiClient.ConnectionCallbacks {
    ...
    /**
     * {@link OwnLocationListener} object
     */

    public OwnLocationListener getmOwnLocationListener;
    /**
    ...
    /**
     * The OwnLocationListener instance used internally to listen for location updates
     */
    private android.location.LocationListener mLocationLocationListener;
    /**
     * The current location with latitude, longitude, speed and altitude
     */
    private Location mPosition;
    private OwnLocationListener mOwnLocationListener;
     ...
     */
    public OwnLocation(final Context context) {
        this(context, false);
    }
     ...
     */
    public OwnLocation(final Context context, final boolean requireFine) {
     ...
     */
    public OwnLocation(final Context context, final boolean requireFine, final boolean passive) {
     ...
     */
    public OwnLocation(final Context context, final boolean requireFine, final boolean passive, final long interval) {
     ...
     */
    public OwnLocation(final Context context, final boolean requireFine, final boolean passive, final long interval, final boolean requireNewLocation) {
    ...
    /**
     * Attaches or detaches a ownLocationListener that informs about certain events
     *
     * @param ownLocationListener the `OwnLocation.OwnLocationListener` instance to attach or `null` to detach
     */
    public void setListener(final OwnLocationListener ownLocationListener) {
        mOwnLocationListener = ownLocationListener;
    }
    ...
    public void beginUpdates() {
        if (mLocationLocationListener != null) {
            endUpdates();
        }
    ...
    public void endUpdates() {
        if (mLocationLocationListener != null) {
            mLocationManager.removeUpdates(mLocationLocationListener);
            mLocationLocationListener = null;
        }
    }
    ...
    /**
     * Creates a new OwnLocationListener instance used internally to listen for location updates
     *
     * @return the new OwnLocationListener instance
     */
    private android.location.LocationListener createLocationListener() {
        return new android.location.LocationListener() {
            ...
            public void onLocationChanged(Location location) {
                ...
                if (mOwnLocationListener != null) {
                    mOwnLocationListener.onPositionChanged();
                }
            }
...
//        Toast.makeText(context, "failed", Toast.LENGTH_SHORT).show();
        ...
        String str = connectionResult.toString();

        Log.d("Connection Failed", "The Connection Failed");
        ...
        if (connectionResult.hasResolution()) {
            try {
                ...
                Log.d("Connection Failed", e.getStackTrace().toString());
            }
        } else {

            switch (result) {
                case ConnectionResult.SERVICE_VERSION_UPDATE_REQUIRED:
                    Toast.makeText(context, "GooglePlayService Is UPDATE REQUIRED!",
                            Toast.LENGTH_LONG).show();
                    MessageOwnLocation.GooglePlayUpdateDialog(context,
                            "Update Google Play Services",
                            "This app won't run unless you ic_update Google Play Services");
                    break;
                case ConnectionResult.SUCCESS://mostafa.azadi:  Play service is available but some other thing is wrong
                    Toast.makeText(context, "GooglePlayService Is Not Working!", Toast.LENGTH_LONG).show();
                    //this.finish();
                    break;
                default:
                    MessageOwnLocation.GooglePlayUpdateDialog(context,
            ...
            public void onResult(@NonNull LocationSettingsResult result1) {
                ...
                switch (status.getStatusCode()) {
                    case LocationSettingsStatusCodes.SUCCESS:
                        mOwnLocationListener.locationOn();
                        break;
    ...
    public void onActivityResult(int result) {
        if (result == Activity.RESULT_OK) {
            ...
            mOwnLocationListener.locationCancelled();
        }
    }
